...
class LoginInstrumentedTest {
    ...
    fun setup() {
        ...
        // Ensure no user is signed in so login screen is shown; restart Activity
        try {
    ...
    fun loginScreen_isDisplayed() {
        ...
        composeRule.waitUntil(30_000) {
            try {
                composeRule.onAllNodesWithTag("loginButton").fetchSemanticsNodes().isNotEmpty()
            } catch (_: Throwable) {
                false
            }
        }

        // Basic smoke: primary Login button exists on startup
